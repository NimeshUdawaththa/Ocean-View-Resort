...
<main>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;">
                ...
                <div style="background:#f0fff4;border:1px solid #b2dfc0;border-radius:10px;padding:18px 20px;">
                    ...
                    <ul style="margin:0;padding-left:18px;line-height:2;color:#2d4a5e;font-size:0.88rem;">
                        ...
                        <li>Click <strong>+ Add Room</strong> to register a new room.</li>
                ...
                <div style="background:#fff0f8;border:1px solid #e0b0d0;border-radius:10px;padding:18px 20px;">
                    <h3 style="margin:0 0 10px;color:#8b1a5a;font-size:1rem;">&#128100; Staff</h3>
                    <ul style="margin:0;padding-left:18px;line-height:2;color:#2d4a5e;font-size:0.88rem;">
                        <li>View all staff accounts: Managers and Reception staff.</li>
                        <li>Filter by role using the <strong>Managers</strong> or <strong>Reception</strong> buttons.</li>
                        <li>Click <strong>+ Add Staff</strong> to create a new user account.</li>
                        <li>Edit existing accounts or reset passwords as needed.</li>
                    </ul>
                </div>
    ...
    <div id="tab-reports" class="tab-pane">
        <!-- Summary Cards -->
        <div id="reportSummaryCards" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:16px;margin-bottom:22px;"></div>
...
function loadReport() {
    ...
    if (to)   params += '&to='   + to;
    $('#reportTableContainer').html('<div class="empty-state"><div class="es-icon">&#8987;</div><p>Loading&hellip;</p></div>');
    $.ajax({
...
function renderReportTable() {
    ...
    if (!rows.length) {
        $('#reportTableContainer').html('<div class="empty-state"><div class="es-icon">&#128202;</div><p>No records found.</p></div>');
        return;
    }
    var statusBadge = function(s) {
        var map = { active:'#1a7a4e:#d4edda', checked_in:'#856404:#fff3cd',
                    checked_out:'#3b28b0:#e8e4ff', cancelled:'#b01a1a:#fde8e8' };
    ...
    var html = '<table><thead><tr>' +
        '<th>#</th><th>Res No.</th><th>Guest</th><th>Contact</th>' +
        '<th>Room Type</th><th>Check-In</th><th>Check-Out</th>' +
        '<th>Nights</th><th>Total (USD)</th><th>Status</th><th>Created By</th></tr></thead><tbody>';
    rows.forEach(function(r, i) {
        html += '<tr>' +
            ...
            '<td style="font-size:12px;color:#6b8a9a;">' + esc(r.createdByName) + '</td>' +
            '</tr>';
    });
    html += '</tbody></table>';
